        ...
        <b-col>
          <b-card no-body>
            <b-card-header class="border-0">
              <h3 class="mb-0">Gerencie as justificativas dos pedidos aqui</h3>
            </b-card-header>
            ...
            <b-row class="justify-content-between align-items-center mx-4">
              <span>{{ justifications.count | decimal }} itens no total</span>

              <b-pagination
                class="ml-auto pt-3"
                v-model="options.pageNumber"
                :total-rows="justifications.count"
...
export default {
  ...
  computed: {
    ...sync("justificationStore", ["justifications", "filter", "loading"]),
  ...
  components: {
    ...
    JustificationModal: () => import("./JustificationModal.vue"),
  },

  watch: {
    params: {
      handler() {
        this.$store.dispatch("justificationStore/get", this.params);
      },
      deep: true,
    },
  },
  ...
  methods: {
    add() {
      this.$store.dispatch("justificationStore/add");
    },

    edit(item) {
      this.$store.dispatch("justificationStore/edit", item);
    },

    remove(item) {
      if (confirm(`Confirma exclusão da justificativa: ${item.descricao}?`)) {
        this.$store.dispatch("justificationStore/remove", item.codigo);
      }
    },
  },
