...
const actions = {
    create: ({ state, dispatch }, item) => {
        state.loading = true
        return axios.post(`${API_URL}/RelClienteTabelaPreco/Create`, item)
            .then((res) => {
                dispatch('get')
    ...
    get: async ({ state }, filter) => {
        const query = new URLSearchParams({
            ...
            OrderBy: filter ? filter.sortBy[0] : 'FkClienteCpfCnpj',
            Sort: filter ? filter.sortDesc[0] ? 'desc' : 'asc' : "asc",
        }).toString()

        state.loading = true
        return axios.get(`${API_URL}/RelClienteTabelaPreco/Get?${query}`)
            .then(res => {
    ...
    remove: ({ state, dispatch }, item) => {
        state.loading = true
        return axios.delete(`${API_URL}/RelClienteTabelaPreco/Remove?${new URLSearchParams(item)}`)
            .then(res => {
                dispatch('get')
